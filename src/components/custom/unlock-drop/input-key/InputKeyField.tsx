...
export const InputKeyField = React.memo<InputKeyFieldProps>(({
    ...
    <div className="relative">
        <Input
            ...
            className="h-10 border-zinc-800 text-sm xs:text-lg md:text-xl md:w-36 lg:w-48 text-center cursor-text pr-5"
            maxLength={8}
            disabled={disabled}
        />
        <button
            ...
            tabIndex={-1}
        >
            {show ? <EyeOff className="size-3 lg:size-4" /> : <Eye className="size-3 lg:size-4" />}
        </button>
    </div>
));
